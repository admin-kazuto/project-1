...
class accModel
{
    ...
    {
        $sql = "SELECT * FROM reset_account WHERE reset_token = '$token' AND reset_expriry > NOW()";
        return $this->conn->query($sql)->rowCount();
    }
    ...
    {
        if ($existingToken) {
            $sql = "UPDATE reset_account SET reset_token = '$otp', reset_expriry = '$expiry', email_send_count = email_send_count + 1 where user_id='$user_id'";
        } else {
