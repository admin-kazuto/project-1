...
class homeModel
{
    ...
    {
        return $this->homeModel->query("SELECT product.product_id,product.product_name,product.description,product.product_image,product_variation.price as price FROM product JOIN category on product.category_id = category.category_id JOIN product_variation on product.product_id = product_variation.variation_id WHERE category.category_id = 2 ")->fetchAll(PDO::FETCH_ASSOC);
    }
